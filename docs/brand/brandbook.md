...
| Где | Текст |
|---|---|
| Welcome (`/start`) | `🧀 Привет, {имя}.\n\nЯ — Сыр. Твой бесплатный AI-бот в Telegram.\n500+ моделей, ноль комиссий.\n\nCatch: нужен свой Puter-токен. Без него — мышеловка пустая.\n\nЖми ниже, чтобы залогиниться.` |
| Login intro (Mini App) | `🔑 Тапни кнопку — откроется Mini App с Puter. Залогинься (или зарегайся), токен прилетит сам.` |
